...
version="1"
version_full="Smile-Catch v."+version
...
def main():
        ...
        try:
                ...
                print("Error: "+str(err)+", Oops wrong input try --help for usage \n\n")
                # usage()
                sys.exit(2)
        ...
        if len(opts) == 0:
            print("                                                 ")
            print("Smile-Catch for GNU/Linux,  See --help for usage")
            print("                                                 ")
            print(" ____            _ _             ____      _       _ ")  
            print("/ ___| _ __ ___ (_) | ___       / ___|__ _| |_ ___| |__ ")
            print("\___ \| '_ ` _ \| | |/ _ \ __  | |   / _` | __/ __| '_ \  ")
            print(" ___) | | | | | | | |  __/ __  | |__| (_| | || (__| | | |  ")
            print("|____/|_| |_| |_|_|_|\___|      \____\__,_|\__\___|_| |_|  ")
            print(" -DEVELOPED BY / ISMAIL AHAMED                             ")
            print("                                                           ") 
            sys.exit()
...
def doListScan(inputList):
    ...
    for Host in ListOfHosts:
       Host.join()
       if Host.Status == -1:
          print(Host.Adress+" NO Smile back :( , offline ")
       else:
          print(Host.Adress+"  SMILE-CATCH ^_^ IN "+str(Host.Status)+"ms")
        ...
        try:
            if (y-x) > 0:
                ...
                for Host in ListOfHosts:
                    Host.join()
                    if Host.Status == -1:
                       print(Host.Adress+" NO Smile-Catch :( , offline")
                    else:
                        print(Host.Adress+" Smile-Catch ^_^ IN "+str(Host.Status)+"ms")
